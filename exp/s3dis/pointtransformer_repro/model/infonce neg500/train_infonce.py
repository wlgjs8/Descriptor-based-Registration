...
def main_worker(gpu, ngpus_per_node, argss):
    ...
    # optimizer1 = torch.optim.SGD(ct_model.parameters(), lr=5e-2, momentum=args.momentum, weight_decay=args.weight_decay)
    optimizer1 = torch.optim.SGD(ct_model.parameters(), lr=1e-2, momentum=args.momentum, weight_decay=args.weight_decay)
    optimizer2 = torch.optim.SGD(scan_model.parameters(), lr=1e-3, momentum=args.momentum, weight_decay=args.weight_decay)
...
def train(train_loader, ct_model, scan_model, criterion, optimizer1, optimizer2, epoch):
    ...
    for i, (ct_image, matched_pair_gradient, coord, feat, target, offset) in enumerate(train_loader):  # (n, 3), (n, c), (n), (b)
        ...
        for ni in range(len(scan_output)):
            rand_indexs = np.random.choice(12000, 500, replace=False)
