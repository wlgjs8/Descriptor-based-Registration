...
def main_worker(gpu, ngpus_per_node, argss):
    ...
    optimizer2 = torch.optim.SGD(scan_model.parameters(), lr=1e-1, momentum=args.momentum, weight_decay=args.weight_decay)
...
def train(train_loader, ct_model, scan_model, criterion, optimizer1, optimizer2, epoch):
    ...
    for i, (ct_image, matched_pair_gradient, coord, feat, target, offset) in enumerate(train_loader):  # (n, 3), (n, c), (n), (b)
        ...
        # neg_ctsample = sampled_ct_feature[np.random.choice(24000, 24000)]

        loss = criterion(scan_anchor, pos_ctsample)
        print('loss : ', loss.shape)
        # loss = criterion(scan_anchor, pos_ctsample, neg_ctsample)
